...
function love.load()
	player = love.graphics.newImage("Woodcutter_idle.png")
	iddle = love.graphics.newImage("Woodcutter_idle.png")
...
function love.update(dt)
    ...
    function love.keypressed (key)
    	if key == 'lshift' and srunning == 0 then   --Run activated!
    	    srunning = 1
    	    running = true
    	    playerSpeed = playerSpeed + runSpeed
    	elseif key == 'lshift' and srunning == 1 then --Run deactivated!
    		srunning = 0
    		running = false
    		playerSpeed = playerSpeed - runSpeed
    	elseif key == 'space' then -- jump
		    fn = 5
		    player = love.graphics.newImage("Woodcutter_jump.png")
		    playerFrame = love.graphics.newQuad(0, 0, 48, 48, player:getDimensions())
		    if fn < 5 then 
		    	fn = 3
		    	player = love.graphics.newImage("Woodcutter_idle.png")
	...
	if love.keyboard.isDown('d') then          -- When the player presses and holds down the "D" button:
		...
		if isMoving == false then 
			fn = 5
			player = love.graphics.newImage("Woodcutter_walk.png")
		...
		if running == true then
			fn = 5
			player = love.graphics.newImage("Woodcutter_run.png")
			isMoving = true
		else
			isMoving = true
			player = love.graphics.newImage("Woodcutter_walk.png")
